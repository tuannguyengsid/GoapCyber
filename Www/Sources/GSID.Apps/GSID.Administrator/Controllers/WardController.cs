...
{
    public class WardController : BaseAuthenticationController
    {
        ...
        // GET: District
        public WardController(IDistrictService _districtService,
        ...
        {
            WardFilterViewModel model = new WardFilterViewModel();
        ...
        public async Task<ActionResult> PartialIndex(string p, string Keyword, string BeginAddDateString, string EndAddDateString, string[] CountryId, string[] ProvinceId, string[] DistrictId)
        {
            var _beginAddDate = ConvertDateTimeIsNull(BeginAddDateString);
            var _endAdddDate = ConvertDateTimeIsNull(EndAddDateString);
            var model = wardService.GetAllBySearch(Keyword, _beginAddDate, _endAdddDate, CountryId, ProvinceId, DistrictId);
        ...
        {
            WardCreateViewModel model = new WardCreateViewModel();
            model.Districts = districtService.GetAllVn();
        ...
        [HttpPost]
        [RBAC]
        public ActionResult Create([Bind(Include = "NameVn, NameEn, DistrictId, KeywordSync, IsDeleted, IsDefault")]WardCreateViewModel obj)
        {
            ...
            try
            {
                if (!RBACUser.HasPermission("RecycleBin", "Ward"))
                    ModelState.Remove("IsDeleted");
                if (ModelState.IsValid)
                {
                    Ward model = Mapper.Map<WardCreateViewModel, Ward>(obj);
                    model.IsDeleted = !RBACUser.HasPermission("RecycleBin", "Ward") ? false : !obj.IsDeleted;
                    model.AddedBy = GSIDSessionFacade.GSIDSessionUserLogon.Id;
                    id = wardService.Create(model);
        ...
        {
            var model = Mapper.Map<Ward, WardEditViewModel>(wardService.GetBy(gsid));
            if (model == null)
        ...
        [HttpPost]
        [RBAC]
        public ActionResult Update([Bind(Include = "Id, NameVn, NameEn, DistrictId, IsDeleted")]WardEditViewModel obj)
        {
            string title    = Message.TITLE_ERROR;
            string message  = Message.CONTENT_POSTDATA_DEFAUT_ERROR;
            string status   = Default.Status_Error;
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    var model = wardService.GetBy(obj.Id);
                    if (model != null)
                    {
        ...
        public ActionResult Delete(string id)
        {
            ...
            var _hasDelete = wardService.Delete(id);
            if (_hasDelete)
        ...
        public ActionResult RecycleBin(string id, bool isDeleted)
        {
            ...
            var _hasRecycleBin = wardService.GetBy(id);
            ...
            wardService.Update(_hasRecycleBin);
