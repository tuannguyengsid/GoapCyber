...
{
    public class RecruitmentTagController : BaseAuthenticationController
    {
        private readonly IRecruitmentTagService recruitmentTagService;
        private readonly IRecruitmentService recruitmentService;
        // GET: RecruitmentTag
        public RecruitmentTagController(IRecruitmentTagService _recruitmentTagService,
        ...
        {
            RecruitmentTagFilterViewModel model = new RecruitmentTagFilterViewModel();
        ...
        public async Task<ActionResult> PartialIndex(string p, string Keyword, string BeginAddDateString, string EndAddDateString)
        {
            ...
            var model = recruitmentTagService.GetAllBySearch(Keyword, _beginAddDate, _endAdddDate);
        ...
        {
            RecruitmentTagCreateViewModel model = new RecruitmentTagCreateViewModel();
        ...
        [HttpPost]
        [RBAC]
        public ActionResult Create([Bind(Include = "NameVn, NameEn, BackgroundColor, TextColor, Sort, IsDeleted")] RecruitmentTagCreateViewModel obj)
        {
            ...
            try
            {
                if (!RBACUser.HasPermission("RecycleBin", "RecruitmentTag"))
                    ModelState.Remove("IsDeleted");
                if (ModelState.IsValid)
                {
                    RecruitmentTag model = Mapper.Map<RecruitmentTagCreateViewModel, RecruitmentTag>(obj);
                    model.NameVn = !string.IsNullOrEmpty(obj.NameVn) ? obj.NameVn.Trim() : "";
                    model.NameEn = !string.IsNullOrEmpty(obj.NameEn) ? obj.NameEn.Trim() : "";
                    model.IsDeleted = !RBACUser.HasPermission("RecycleBin", "RecruitmentTag") ? false : !obj.IsDeleted;
                    model.AddedBy = GSIDSessionFacade.GSIDSessionUserLogon.Id;
                    id = recruitmentTagService.Create(model);
        ...
        {
            var model = Mapper.Map<RecruitmentTag, RecruitmentTagEditViewModel>(recruitmentTagService.GetBy(gsid));
            if (model == null)
        ...
        [HttpPost]
        [RBAC]
        public ActionResult Update([Bind(Include = "Id, NameVn, NameEn, BackgroundColor, TextColor, IsDeleted, Sort")] RecruitmentTagEditViewModel obj)
        {
            string title = Message.TITLE_ERROR;
            string message = Message.CONTENT_POSTDATA_DEFAUT_ERROR;
            string status = Default.Status_Error;
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    var model = recruitmentTagService.GetBy(obj.Id);
                    if (model != null)
                    {
                        model.NameVn = !string.IsNullOrEmpty(obj.NameVn) ? obj.NameVn.Trim() : "";
                        model.NameEn = !string.IsNullOrEmpty(obj.NameEn) ? obj.NameEn.Trim() : "";
                        model.BackgroundColor   = obj.BackgroundColor;
                        model.TextColor         = obj.TextColor;
                        model.Sort              = obj.Sort;
                        if (RBACUser.HasPermission("RecycleBin", "RecruitmentTag"))
                            model.IsDeleted = !obj.IsDeleted;
                        model.EditedBy = GSIDSessionFacade.GSIDSessionUserLogon.Id;
                        recruitmentTagService.Update(model);
        ...
        public ActionResult Delete(string id)
        {
            ...
            var recrs = recruitmentService.GetAllByRecruitmentTag(id);
            if (recrs.Count <= 0)
            {
                var _hasDelete = recruitmentTagService.Delete(id);
                status = ((int)StatusDelete.Deleted).ToString();
            }
            else
            {
                message = string.Format(Message.CONTENT_POSTDATA_DELETE_ERROR_OR_ERROR_EXIST_DATA, "Thẻ tin tuyển dụng", "tin tuyển dụng");
            }
        ...
        public ActionResult RecycleBin(string id, bool isDeleted)
        {
            ...
            var _hasRecycleBin = recruitmentTagService.GetBy(id);
            ...
            recruitmentTagService.Update(_hasRecycleBin);
        ...
        public JsonResult IsNameVnAvailable(string NameVn)
        {
            ...
            return Json(recruitmentTagService.IsNameVnAvailable(NameVn) != null ? false : true, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            // Check if the NameVn already exists
            return Json(recruitmentTagService.IsNameVnAvailable(NameVn, Id) != null ? false : true, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult IsNameEnAvailable(string NameEn)
        {
            ...
            return Json(recruitmentTagService.IsNameEnAvailable(NameEn) != null ? false : true, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            // Check if the NameEn already exists
            return Json(recruitmentTagService.IsNameEnAvailable(NameEn, Id) != null ? false : true, JsonRequestBehavior.AllowGet);
        }
